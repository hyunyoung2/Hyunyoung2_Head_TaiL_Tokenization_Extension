...
def read_n_gram_dict(path, dict_idx="None"):
    ...
    for idx, val in enumerate(data):
        ...
        if dict_idx == DICT_TYPES[0]:
            ...
            if dict_idx in [DICT_TYPES[1], DICT_TYPES[3]]: # "LEFT_BI", "RIGHT_BI"
...
## New function
def dict_read(loc_of_dict, type_of_dict):
    """ Read several dict according to the type of dict 
    
    Args:
        loc_of_dict(str): The location of dictionary

        type_of_dict(str): one of DICT_TYPES = ["UNI",
                                                "LEFT_BI", LEFT_TRI"",
                                                "RIGHT_BI", "RIGHT_TRI",
                                                :BIDIRECTIONAL_BI_WINDOW_1, "BIDIRECTIONAL_TRI_WINDOW_2"]

    Returns:
        dicts(dict): uni_dict, left_bi_dict, right_bi_dict, left_tri_dict, right_tri_dict
    """

    temp_dict_path = loc_of_dict.split("/")
    final_dict_path = "/".join(temp_dict_path[:-1])+"/"

    if DEBUG in DEBUG_LEVEL[0:]:
        print("Temp_dict_path: {}".format(temp_dict_path))
        print("Final_dict_path: {}".format(final_dict_path))

    total_dict = OrderedDict()

    ##                 0  ,     1    ,      2    ,      3    ,      4     ,              5             ,               6
    ## DICT_TYPES = ["UNI", "LEFT_BI", "LEFT_TRI", "RIGHT_BI", "RIGHT_TRI", "BIDIRECTIONAL_BI_WINDOW_1", "BIDIRECTIONAL_TRI_WINDOW_2"]
    if DICT_TYPES[0] == type_of_dict: # UNI
       _, _, uni_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[0], dict_idx=DICT_TYPES[0])

       total_dict[DICT_TYPES[0]] = uni_dict

    elif DICT_TYPES[1] == type_of_dict: # LEFT_BI
       _, _, uni_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[0], dict_idx=DICT_TYPES[0])
       _, _, left_bi_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[1], dict_idx=DICT_TYPES[1])

       total_dict[DICT_TYPES[0]] = uni_dict
       total_dict[DICT_TYPES[1]] = left_bi_dict

    elif DICT_TYPES[2] == type_of_dict: # LEFT_TRI
       _, _, uni_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[0], dict_idx=DICT_TYPES[0])
       _, _, left_bi_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[1], dict_idx=DICT_TYPES[1])
       _, _, left_tri_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[2], dict_idx=DICT_TYPES[2])

       total_dict[DICT_TYPES[0]] = uni_dict
       total_dict[DICT_TYPES[1]] = left_bi_dict
       total_dict[DICT_TYPES[2]] = left_tri_dict

    elif DICT_TYPES[3] == type_of_dict: # RIGHT_BI
       _, _, uni_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[0], dict_idx=DICT_TYPES[0])
       _, _, right_bi_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[3], dict_idx=DICT_TYPES[3])

       total_dict[DICT_TYPES[0]] = uni_dict
       total_dict[DICT_TYPES[3]] = right_bi_dict

    elif DICT_TYPES[4] == type_of_dict: # RIGHT_TRI
       _, _, uni_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[0], dict_idx=DICT_TYPES[0])
       _, _, right_bi_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[3], dict_idx=DICT_TYPES[3])
       _, _, right_tri_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[4], dict_idx=DICT_TYPES[4])

       total_dict[DICT_TYPES[0]] = uni_dict
       total_dict[DICT_TYPES[3]] = right_bi_dict
       total_dict[DICT_TYPES[4]] = right_tri_dict

    elif DICT_TYPES[0] == type_of_dict: # BIDIRECTIONAL_BI_WINDOW_1 
       assert False, "You don't need to check BIDIRECTIONAL_BI_WINDOW_1"
       pass
       ## this one is not implement yet
       ##_, _, uni_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[0], dict_idx=DICT_TYPES[0])
       ##_, _, left_bi_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[2], dict_idx=DICT_TYPES[2])
       ##_, _, right_bi_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[3], dict_idx=DICT_TYPES[3])
       ##_, _, bi_bi_dict = read_n_gram_dict(path=final_dict_path+DICT_TYPES[5], dict_idx=DICT_TYPES[5])

       ##total_dict[DICT_TYPES[0]] = uni_dict
       ##total_dict[DICT_TYPES[2]] = left_bi_dict
       ##total_dict[DICT_TYPES[3]] = right_bi_dict
       ##total_dict[DICT_TYPES[5]] = right_tri_dict

    elif DICT_TYPES[0] == type_of_dict: # BIDIRECTIONAL_BI_WINDOW_2
       assert False, "You don't need to check BIDIRECTIONAL_BI_WINDOW_2"

    return total_dict       


## for n-gram Search dict 
def match_with_total_dict(input_sent, ngram_data, total_n_gram_dict_data, _ground_truth, dict_types="NONE"):
    """matching n_gram_word with total_n_gram_dict 
    ...
    dict is pair_dict which consists of the following:
               ...
               {n-gram_0: [(head_tail_token_1, cnt, prob), (head_tail_token_2, cnt, prob), ..., 
                n_gram_1: [(head_tail_token_1, cnt, prob), (head_tail_token_2, cnt, prob)]

    This code is needed to be arranged
    ...
    assert dict_types != "NONE" and dict_types != "UNI", "Choose dict type('{}') for matching function, 'match_with_total_dict'".format(dict_types)
    assert dict_types in DICT_TYPES[1:5], "Choose dict type('{}') for matching function, 'match_with_total_dict'".format(dict_types)
    ...
    # I have to refactor the code lines below, But Currently tims is short
    for idx, val in enumerate(ngram_data):
         ##print(total_n_gram_dict_data[dict_types].get(val))
         ##print(input_sent)
         if dict_types == DICT_TYPES[1]: # LEFT_BI
            ## key = (CONTEXT1, TOKEN) 
            ##print("\nLEFT_BI")
            ##print("Key: {}".format(val))
            ##if total_n_gram_dict_data["UNI"].get(val[-1]):
            ##    print("Uni: {}".format(total_n_gram_dict_data["UNI"][val[-1]]))
            ##if total_n_gram_dict_data["LEFT_BI"].get(val):
            ##    print("LEFT_Bi: {}".format(total_n_gram_dict_data["LEFT_BI"][val]))
            if total_n_gram_dict_data["UNI"].get(val[-1]) != None:
                temp_uni = total_n_gram_dict_data["UNI"][val[-1]]
                if len(temp_uni) >= 2:
                    if total_n_gram_dict_data[dict_types].get(val) != None:
                        temp_left_bi = total_n_gram_dict_data[dict_types][val]
                        if len(temp_left_bi) >= 2:
                            print("Key: {}, {}: {}".format(val, dict_types, temp_left_bi))
                        answer.append(temp_left_bi[0][0])
                    else:
                        answer.append(temp_uni[0][0])
                else:
                    answer.append(temp_uni[0][0])
            else:
                original_word_num += 1
         ...
         elif dict_types == DICT_TYPES[3]: # RIGHT_BI
            ## (TOKEN, CONTEXT1)
            ##print("\nRIGHT_BI")
            ##print("Key: {}".format(val))
            ##if total_n_gram_dict_data["UNI"].get(val[0]):
            ##    print("Uni: {}".format(total_n_gram_dict_data["UNI"][val[0]]))
            ##if total_n_gram_dict_data["RIGHT_BI"].get(val):
            ##    print("Right_Bi: {}".format(total_n_gram_dict_data["RIGHT_BI"][val]))
            if total_n_gram_dict_data["UNI"].get(val[0]) != None:
                temp_uni = total_n_gram_dict_data["UNI"][val[0]]
                if len(temp_uni) >= 2:
                    if total_n_gram_dict_data[dict_types].get(val) != None:
                        temp_right_bi = total_n_gram_dict_data[dict_types][val]
                        if len(temp_right_bi) >= 2:
                            print("Key:{}, {}: {}".format(val, dict_types, temp_right_bi))
                        answer.append(temp_right_bi[0][0])
                    else:
                        answer.append(temp_uni[0][0])
                else: 
                    answer.append(temp_uni[0][0])
         ...
         elif dict_types == DICT_TYPES[2]: # LEFT_TRI
            ## (CONTEXT2, CONTEXT1, TOKEN)
            ##print("\nLEFLT_TRI")
            ##print("Key: {}".format(val))
            ##if total_n_gram_dict_data["UNI"].get(val[-1]):
            ##    print("Uni: {}".format(total_n_gram_dict_data["UNI"][val[-1]]))
            ##if total_n_gram_dict_data["LEFT_BI"].get(val[1:]):
            ##    print("Left_Bi: {}".format(total_n_gram_dict_data["LEFT_BI"][val[1:]]))
            ##if total_n_gram_dict_data["LEFT_TRI"].get(val):
            ##    print("Left_Tri: {}".format(total_n_gram_dict_data["LEFT_TRI"][val]))
            if total_n_gram_dict_data["UNI"].get(val[-1]) != None:
                temp_uni = total_n_gram_dict_data["UNI"][val[-1]]
                if len(temp_uni) >= 2:
                    if total_n_gram_dict_data["LEFT_BI"].get(val) != None:
                        temp_left_bi = total_n_gram_dict_data["LEFT_BI"][val[1:]]
                        if len(temp_left_bi) >= 2:
                            temp_left_tri = total_n_gram_dict_data[dict_types][val]
                            if len(temp_left_tri) >= 2:
                                 print("key:{}, {}: {}".format(val, dict_types, temp_left_tri))
                            answer.append(temp_left_tri[0][0])
                        else:
                            answer.append(temp_left_bi[0][0])
                    else:
                        answer.append(temp_uni[0][0])
                ...
                if _ground_truth[idx] == original_sent[idx]:
                    ...
                    ##input()
                answer.append(original_sent[idx])
         elif dict_types == DICT_TYPES[4]: # RIGHT_TRI
            ## (TOKEN, CONTEXT1, CONTEXT2)
            ##print("\nRIGHT_TRI")
            ##print("Key: {}".format(val))
            ##if total_n_gram_dict_data["UNI"].get(val[0]):
            ##    print("Uni: {}".format(total_n_gram_dict_data["UNI"][val[0]]))
            ##if total_n_gram_dict_data["RIGHT_BI"].get(val[0:2]):
            ##    print("Right_Bi: {}".format(total_n_gram_dict_data["RIGHT_BI"][val[0:2]]))
            ##if total_n_gram_dict_data["RIGHT_TRI"].get(val):
            ##    print("Right_Tri: {}".format(total_n_gram_dict_data["RIGHT_TRI"][val]))
            if total_n_gram_dict_data["UNI"].get(val[0]) != None:
                temp_uni = total_n_gram_dict_data["UNI"][val[0]]
                if len(temp_uni) >= 2:
                    if total_n_gram_dict_data["RIGHT_BI"].get(val) != None:
                        temp_right_bi = total_n_gram_dict_data["RIGHT_BI"][val[0:2]]
                        if len(temp_right_bi) >= 2:
                            temp_right_tri = total_n_gram_dict_data[dict_types][val]
                            if len(temp_right_tri) >= 2:
                                 print("key:{}, {}: {}".format(val, dict_types, temp_right_tri))
                            answer.append(temp_right_tri[0][0])
                        else:
                            answer.append(temp_right_bi[0][0])
                    else:
                        answer.append(temp_uni[0][0])
            ...
            assert False, "Your searching n-gram dict is wrong"
